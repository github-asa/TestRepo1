...
namespace J2BIOverseasOps.Pages.BuildingWork.Nearby.Bars
{
    class NearbyBarsPage : BuildingWorkCommon
    {
        //affected bars
        ...
        public NearbyBarsPage(IWebDriver driver, ILog log, IRunData rundata) : base(driver, log, rundata)
        {
            _apiCall = new ApiCalls(rundata);

        }

        public void VerifyNearbyBarsMandatoryMessage(Table table)
        {
        ...
        public void EnterNearbyBarsAnswers(Table table)
        {
            foreach (var row in table.Rows)
            {
