...
namespace J2BIOverseasOps.Pages.BuildingWork.Nearby.Pools
{
    class NearbyPoolsPage : BuildingWorkCommon
    {
        //affected pools
        ...
        public NearbyPoolsPage(IWebDriver driver, ILog log, IRunData rundata) : base(driver, log, rundata)
        {
            _apiCall = new ApiCalls(rundata);

        }

        public void VerifyNearbyPoolsMandatoryMessage(Table table)
        {
        ...
        public void EnterNearbyPoolsAnswers(Table table)
        {
            foreach (var row in table.Rows)
            {
