...
namespace J2BIOverseasOps.Pages.BuildingWork.Nearby.Restaurants
{
    class NearbyRestaurantsPage : BuildingWorkCommon
    {
        ...
        // page buttons  
        private readonly By _removeThisRestaurant = By.XPath("//p-button[@id='removeThisItem']//button");
        private readonly By _addThisRestaurant = By.XPath("//p-button[@id='addAnotherItem']//button");

        private readonly ApiCalls _apiCall;

        public NearbyRestaurantsPage(IWebDriver driver, ILog log, IRunData rundata) : base(driver, log, rundata)
        {
            _apiCall = new ApiCalls(rundata);
        }

        public void VerifyNearbyRestaurantsMandatoryMessage(Table table)
        {
        ...
        public void EnterNearbyRestaurantsAnswers(Table table)
        {
            foreach (var row in table.Rows)
            {
        ...
        public void VerifyOnSiteRestaurantsAnswers(Table table)
        {
            foreach (var row in table.Rows)
            {
                ...
                {
                    case "affected restaurant":
                        Driver.VerifySelectedDropDownOption(_affectedRestaurantsDropDown, GetAffectedRestaurant(answer));
                        break;
                    case "how is restaurant affected":
                    ...
                    case "work affect board basis details":
                        ...
                        break;
                    case "name of the restaurant":
                        Driver.VerifyInputBoxText(_otherRestaurantName, answer);
                        break;
                    case "hotel providing alternative arrangements":
