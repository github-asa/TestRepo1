...
namespace J2BIOverseasOps.Pages.BuildingWork.Nearby.Rooms
{
    internal class NearbyRoomsPage : BuildingWorkCommon
    {
        private readonly ApiCalls _apiCall;

        public NearbyRoomsPage(IWebDriver driver, ILog log, IRunData runData) : base(driver, log,runData)
        {
        ...
        public void EnterAnswerToNearbyRooms(Table table)
        {
            foreach (var row in table.Rows)
            {
                ...
                switch (question.ToLower())
                {
                    ...
                    case "details on types of room affected":
                        ...
                        break;
                    default:
                        Assert.Fail($"{question} is not a valid Nearby Rooms question");
                        return;
                }
            }
        }

        public void VerifyNearbyQDisplayed(string displayedNotDisplayed, string question)
        {
