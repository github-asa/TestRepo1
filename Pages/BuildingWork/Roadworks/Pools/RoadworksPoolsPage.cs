...
namespace J2BIOverseasOps.Pages.BuildingWork.Roadworks.Pools
{
    class RoadworksPoolsPage : BuildingWorkCommon
    {
        //affected pools
        ...
        private readonly By _howPoolsAffectedDetails = By.XPath("//textarea[@id='moreDetails']");
        private readonly By _howPoolsAffectedDetailsValidation = By.XPath("//p-message[@id='moreDetails-validation']");
        ...
        // page buttons  
        private readonly By _removeThisPool = By.XPath("//p-button[@id='removeThisPool']//button");
        private readonly By _addThisPool = By.XPath("//p-button[@id='addAnotherPool']//button");

        private readonly ApiCalls _apiCall;

        public RoadworksPoolsPage(IWebDriver driver, ILog log, IRunData rundata) : base(driver, log, rundata)
        {
            _apiCall = new ApiCalls(rundata);

        }

        public void VerifyRoadworksPoolsMandatoryMessage(Table table)
        {
        ...
        public void EnterRoadworksPoolsAnswers(Table table)
        {
            foreach (var row in table.Rows)
            {
            ...
            foreach (var row in table.Rows)
            {
                ...
                switch (question.ToLower())
                {
                    case "affected pool":
                        ...
                        break;
                    case "how is pool affected":
                        Driver.VerifySelectedDropDownOption(_howPoolsAffectedDrpDwn, answer);
                        break;
                    case "how is pool affected details":
        ...
        {
            var poolList = pool.ConvertStringIntoList(); // converts expected restaurants to a List
            ...
            var excludePools = poolIndex.Select(i => ListOfPools[i - 1]).ToList();
            if (excludesIncludes.ToLower() == "excludes")
            {
                for (var i = 0; i < excludePools.Count(); i++)
                {
                    Assert.True(listOfActualPools.Contains(excludePools[i]) == false);
                }
            }
            else if (excludesIncludes.ToLower() == "includes")
            {
                for (var i = 0; i < excludePools.Count(); i++)
                {
                    Assert.True(listOfActualPools.Contains(excludePools[i]));
                }
            }
        }
    }
